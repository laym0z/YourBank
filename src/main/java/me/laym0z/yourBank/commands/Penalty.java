...
public class Penalty implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, String[] args) {
        ...
        if (args.length == 0) {
            commandSender.sendMessage(ChatColor.RED+"[Штрафи] Введи дані");
            return true;
        }
        if (args.length < 5 && args[0].equals("add")) {
            commandSender.sendMessage(ChatColor.RED+"[Штрафи] Дані введені не вірно");
            return false;
        }
        //ADD
        if (args.length >= 4 && args[0].equals("add")) {
            ...
            if (!Data.hasPlayedBefore(name)) {
                commandSender.sendMessage(String.format(ChatColor.RED+"[Штрафи] Гравця %s не існує!", name));
                return true;
            }
            ...
            try {
                ...
                commandSender.sendMessage(ChatColor.RED+"[Штрафи] Некорекне поле суми: "+ChatColor.UNDERLINE+args[2]);
                return true;
            }
            if (!termPayment.matches("^\\d{2}-\\d{2}-\\d{4}$")){
                commandSender.sendMessage(ChatColor.RED+"[Штрафи] Дата має вводитися в наступному форматі: DD-MM-YYYY \nПриклад: "+ChatColor.GOLD+"01-01-2025");
                return true;
            }
            ...
            try {
                ...
                commandSender.sendMessage(ChatColor.RED + "[Штрафи] Неможливо обробити дату. Перевір формат.");
                return true;
            }

            if (parsedDate.isBefore(LocalDate.now())) {
                commandSender.sendMessage(ChatColor.RED+"[Штрафи] Термін оплати не може бути раніше поточної дати!");
                return true;
            }
            ...
            commandSender.sendMessage(String.format(ChatColor.GREEN+"[Штрафи] Гравцю %s був виписаний штраф на суму %d ДР", name, amount));
            return true;
        }
        //LIST
        else if (args.length == 2 && args[0].equals("list")) {
            if (!Data.doesPlayerHavePenalties(args[1])) {
                commandSender.sendMessage(ChatColor.RED+"[Штрафи] Цей гравець не має штрафів");
                return true;
            }
...
//            }
            ...
            commandSender.sendMessage(ChatColor.RED+"[Штрафи] Занадто багато аргументів!");
            return true;
        }
        else {
            commandSender.sendMessage(ChatColor.RED+"[Штрафи] Неправельні аргументи");
            return true;
        }
        return true;
    }
